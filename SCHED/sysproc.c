...
sysexec(Ar0* ar0, va_list list)
{
	...
	up->seg[ESEG] = newseg(SG_STACK, TSTKTOP-USTKSIZE, TSTKTOP, nil, 0);
	...
	*--argv = nil;
	...
	img = attachimage(chan);
	if(waserror()){
		putimage(img);
		nexterror();
	}
	if(img->section[0] == nil)
		img->section[0] = newsection(textmin-UTZERO, 0, hdrsz+textsz);
	if(img->section[1] == nil)
		img->section[1] = newsection(datalim-textlim, hdrsz+textsz, datasz);
	unlock(img);

	ts = newseg(SG_TEXT|SG_RONLY, UTZERO, textmin, img, 0);
	if(waserror()){
		putseg(ts);
		nexterror();
	}
	ts->flushme = 1;
	ds = newseg(SG_DATA, textlim, datalim, img, 1);
	if(waserror()){
		putseg(ds);
		nexterror();
	}
