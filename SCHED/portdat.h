...
#define QUALIFY_TO_MIGRATE	1
#define HAVE_MIGRATED		2
...
struct Dev
{
	...
	long	(*readv)(Chan*, IOchunk*, long, long, vlong);
	long	(*writev)(Chan*, IOchunk*, long, long, vlong);
};
...
struct Page
{
	...
	uchar	mdom;		/* Memory domain (could probably replace by a pa2dom map) */
	Page	*next;		/* Free list */
	PPAGE;
};
...
//subtype
	Section	*section[2];		/* TEXT and DATA prototypes of image if running or cached */
	Image	*hash;			/* Qid hash chains */
	Image	*next;			/* LRU free list */
	Image	*prev;
};

struct Section
{
	QLock	lk;			/* lock for update */
	uintptr	xsize;		/* size in bytes */
	usize	npages;		/* size in pages */
	ulong	fstart;		/* start address in file for demand load */
	ulong	flen;		/* length of segment in file */
	uchar	lg2pgsize;	/* log2(size of pages in set) */
	Page*	pages[];	/* Pages assigned, once loaded; entry is nil if still on file */
};
...
struct Segment
{
	...
	Pages*	pages;		/* physical pages mapped to virtual base */
	Image*	image;		/* prototype in file attached to this segment */
	int	isec;		/* Section index in Image */
	Physseg *pseg;
	Lock	semalock;
	Sema	sema;
	ulong*	profile;
...
#define	segpgsize(s)	(1<<(s)->pages->lg2pgsize)
...
#define REND(p,s)	((p)->rendhash[(uintptr)(s)%RENDHASH])
