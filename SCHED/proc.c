...
Lock voting_lock;
ulong remig_next;
ulong remig_timeout_interval = 1000;
int climbing = 1;
int thro_last_step;
...
void VOTE(int num, int num_tasks, ulong locking_time, ulong total_len)
{
	voting_count += num;
	voting_rqlen += num_tasks;
	voting_locking += locking_time;
	voting_slice += total_len;	
}
void CLEAR_VOTING(void)
{
	voting_count = 0;
	voting_slice = 0;
	voting_locking = 0;
...
void RECORD_LAST_VOTING(int num_tasks, int cores, int thro)
{
	num_tasks_last_step = num_tasks;
	work_cores_last_step = cores;
	thro_last_step = thro;
}
void SHRINK_VOTING(int times, int div)
{
	voting_count = voting_count*times/div;
	voting_slice = voting_slice*times/div;
	voting_locking = voting_locking*times/div;
	voting_rqlen = voting_rqlen*times/div;
}

int timer_fires(ulong goal)
{
	if (rdtsc() > goal) return 1;
	return 0;
}
...
void determine_new_bound(void)
{
	int moving = 0;
	int useful_perc = 1024-1024*voting_locking/voting_slice;
	int work_cores = (n_lock_intensive_tasks > special_core_bound)? special_core_bound:n_lock_intensive_tasks;
	int thro = useful_perc*work_cores/1024;
	int new_bound;
	if (climbing) {
		if (thro > thro_last_step) {
			if (climbing<0) climbing = 1;
			else {
				if (2 == climbing) {
					new_bound = 2*work_cores;
					if (new_bound > max_special_cores)
						new_bound = max_special_cores;
					if (new_bound > n_lock_intensive_tasks) 
						new_bound = n_lock_intensive_tasks;
					if (new_bound > work_cores) {
						special_core_bound = new_bound;
			...
			if (climbing > 0) climbing = -1;
				...
				if (climbing == -2) {
					special_core_bound=work_cores_last_step;
					CLEAR_VOTING();
					moving = 1;
					climbing = 0;
		...
		if (work_cores!=work_cores_last_step || thro > 2*thro_last_step || thro < thro_last_step/2) 
			remig_next = rdtsc();	
	} 	
	if (0 == moving) SHRINK_VOTING(1,2);
	else remig_next = rdtsc() + remig_timeout_interval;
}
