...
/*
 * TO DO: this is not the usual meaning of scatter/gather
 */
static long
deviov(Chan *c, IOchunk *io, long nio, vlong offset, int mode)
{
	long i, n, len;
	long (*fn)(Chan*, void*, long, vlong);

	fn = mode == OWRITE? c->dev->write: c->dev->read;
	len = 0;
	for(i = 0; i < nio; i++) {
		n = fn(c, io[i].addr, io[i].len, offset);
		if(n != io[i].len)
			break;
		offset += n;
		len += n;
...
long
devreadv(Chan *c, IOchunk *io, long nio, long, vlong offset)
{
	return deviov(c, io, nio, offset, OREAD);
}

long
devwritev(Chan *c, IOchunk *io, long nio, long, vlong offset)
{
	return deviov(c, io, nio, offset, OWRITE);
}
